...
import { InfoPeli, Result, Resultados } from '@/api/type';
    ...
    useEffect(() => {
        ...
        const Escenas = async () => {
           ...
           const videos = Object.values(films).map(async( item) =>{
                ...
                const movieTrailers = videosRes?.filter(
                    (video )=> {
                        const release_date = video.published_at && new Date(video.published_at) ;                       
                        return video.type === "Trailer" && video.site === "YouTube" &&  release_date >= week && release_date <= now
              ...
              <Card className="w-[150px] xl:w-[200px] xl:h-[250px]">
                <CardHeader className="p-0">
                  <Link to={`/movies/${item.id} `}>
                    <img src={`https://image.tmdb.org/t/p/w500/${item.poster_path}`} alt="" className="rounded-[0.75rem] w-full h-[200px] xl:h-[250px]" />
                  </Link>
                </CardHeader>
              </Card>
